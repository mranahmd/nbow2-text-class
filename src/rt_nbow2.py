...
def train_mlp(
    ...
    saveto='rt_nbow2_fold0-model.npz',  # The best model will be saved there
    outDir=None,
    ...
    if outDir:
	saveto= outDir+ "/rt_nbow2_fold" + str(fold) + "-model.npz"
    else:
    ...
    test_err = pred_error(f_pred, prepare_data, test, kf_test)
    tmp = get_wWts(f_pred_prob, prepare_data, test, kf_test, model_options, outDir+"/rt_nbow2_fold"+str(fold)+".test-")
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
	train_mlp(max_epochs=50, input_path=sys.argv[1], outDir=sys.argv[2], fold=int(sys.argv[3]))
    else:
	print "USAGE: ", sys.argv[0], "<input_dir>", "<output_dir>", "<fold_0-9>"
