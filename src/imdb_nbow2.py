...
def get_wWts(f_pred_prob, prepare_data, data, iterator, options, outPrefix, verbose=False):
    valid_err = 0
    sys.stderr.write('\n------------------------------------------------------------------\n')

    wDict = []
    with open(options['input_path'] + "/vocab", "r") as ins:
	for line in ins:
		w = line.strip()
		wDict.append(w)
	...
	for i in range (0, len(y)):
		for k in range(0, options['num_anch_vec']):
			for j in range(0, wWts.shape[0]):
				if mask[j][i] == 1:
					if k == 0:
						wtFile.write(str(wWts[j][i][k]) +' ')
						tiFile.write(str(x[j][i]) +' ')
						wNwtFile.write(wDict[x[j][i]] + '_'+ str(wWts[j][i][k]) + ' ')
		wtFile.write('\n')
		tiFile.write('\n')
		wNwtFile.write('\n')
...
def train_mlp(
    ...
    if outDir:
	saveto= outDir+ "/imdb_nbow2_model.npz"
    else:
	...
	for line in f:
		...
		bow = [int(x) for x in line.split()]
        	data_x.append(bow)
		data_y.append(0)
    with open(input_path + '/valid-pos.id') as f:		### using hyperparameters chosen by Iyyer (2015), so this is part of train; as in their experiments
	for line in f:
		bow=[]
		bow = [int(x) for x in line.split()]
        	data_x.append(bow)
		data_y.append(1)
    with open(input_path + '/valid-neg.id') as f:		### using hyperparameters chosen by Iyyer (2015), so this is part of train; as in their experiments
	for line in f:
		bow=[]
		bow = [int(x) for x in line.split()]
        	data_x.append(bow)
    ...
    print ('Staring Error: Test ', test_err)
        ...
        for eidx in xrange(max_epochs):
            ...
            for _, train_index in kf:
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    if (uidx == 0 or test_err <= numpy.array(history_errs)[:, 0].min()):
                        best_p = unzip(tparams)
                        bad_counter = 0

                    print ('Train ', train_err, 'Test ', test_err)

                    if (len(history_errs) > patience and test_err >= numpy.array(history_errs)[:-patience, 0].min()): ### Warning: This should be normally used with a separate validation set
                        bad_counter += 1
    ...
    test_err = pred_error(f_pred, prepare_data, test, kf_test)
    tmp = get_wWts(f_pred_prob, prepare_data, test, kf_test, model_options, outDir+"/test-")
...
if __name__ == '__main__':
    if len(sys.argv) == 3:
	train_mlp(max_epochs=50, input_path=sys.argv[1], outDir=sys.argv[2])
    else:
