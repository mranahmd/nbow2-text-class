...
def build_model(tparams, options):
    ...
    proj = embMasked.sum(axis=0) / finalMask.sum(axis=0)[:, None]
    ...
    L2_sqr = tensor.sum(tparams['U'] ** 2) + tensor.sum(tparams['b'] ** 2) + tensor.sum(embMasked ** 2)
    ...
    f_ops = theano.function([x, mask], proj, name='f_ops')
...
def train_mlp(
    ...
    saveto='20ng_nbow_model.npz',  # The best model will be saved there
    outDir=None,
    ...
    if outDir:
	saveto= outDir+ "/20ng_nbow_model.npz"
    else:
